...
logger.warning('DB load')

def run_dist(signature_id,job_id,user_id):
	logger.warning('Run dist')
	try :
		log = ""
		...
		if user_id == 'None':
			...
			if os.path.isfile(os.getcwd()+'/var/Data/Public/Database/'+signature['projects']+'/'+signature['id']+'/'+signature['file_up']):
				...
				fileAdmin.close()
			if os.path.isfile(os.getcwd()+'/var/Data/Public/Database/'+signature['projects']+'/'+signature['id']+'/'+signature['file_interrogated']):
				fileAdmin = open(os.getcwd()+'/var/Data/Public/Database/'+signature['projects']+'/'+signature['id']+'/'+signature['file_interrogated'],'r')
				for linesFile in fileAdmin.readlines():
					check_files.write(linesFile.replace('\n','')+'\t'+str(0)+'\n')
				fileAdmin.close()
			if os.path.isfile(os.getcwd()+'/var/Data/Public/Database/'+signature['projects']+'/'+signature['id']+'/'+signature['file_down']):
				fileAdmin = open(os.getcwd()+'/var/Data/Public/Database/'+signature['projects']+'/'+signature['id']+'/'+signature['file_down'],'r')
				for linesFile in fileAdmin.readlines():
					check_files.write(linesFile.replace('\n','')+'\t'+'-1'+'\n')
				fileAdmin.close()
		...
		if user_id != 'None':
			...
			for signature in all_signature_private :
				...
				if signature['status'] == 'public':
					print os.path.isfile(os.getcwd()+'/var/Data/Public/Database/'+signature['projects']+'/'+signature['id']+'/'+signature['file_up'])
		...
		log = log + 'Run script distance analysis\n'
		db['Jobs'].update({'id': int(job_id)},{'$set':{'stderr':log}})
